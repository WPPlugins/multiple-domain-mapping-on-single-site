...
! BUG: missing trailing slashes (https://wordpress.org/support/topic/child-page-of-domain/)
! BUG: defined /mapping-url gets active for an url like /part1/mapping-url
! BUG: https-only one line needed?
! Elementor-Compatibility (Elementor does not load, maybe since logged out?)
! Options switch to enable "Include Path Operations"?
! FAQ: http -> https and non-www -> www sample code for .htaccess (check for [multiple] subdomains)
! FAQ: when and how to use $falke_mdm_currDomain; (eg for assets; better to use get_stylesheet_directory_uri or get_template_directory_uri)

+ extra switches to enable url-path changes (difficult with http / https)
+ Post-Pagination (https://wordpress.org/support/topic/nextpage-not-working-2/)
+ Change/Add Cookies to be logged in on other domains?
+ Password Protected Pages (https://wordpress.org/support/topic/password-protected-page-not-working-correctly/)
...
+ Extra Switch for 301 redirections from original source to new domain (use really good description -> only activate after mappings are working)
+ Extra Switch for www redirections (same as above)
+ Extra Switch for SSL redirections (same as above)

~ Extra field per mapping for custom header code (google analytics?)
~ Showcase (?)
...
// remove shortlink from header - will not work...
...
/*---------------------------------------------------------------------------------------------------------------------------
Include Path Operations -> change only domain part
---------------------------------------------------------------------------------------------------------------------------*/
//only enable with options switch!
/*
add_filter( 'script_loader_tag', 'falke_mdm_replace_url_in_tags');
add_filter( 'style_loader_tag', 'falke_mdm_replace_url_in_tags');
add_filter( 'stylesheet_directory_uri', 'falke_mdm_replace_url_in_tags');
add_filter( 'template_directory_uri', 'falke_mdm_replace_url_in_tags');
function falke_mdm_replace_url_in_tags($tag){
	global $falke_mdm_currDomain;
	if(!is_admin()){
		return str_replace(parse_url(get_site_url(), PHP_URL_HOST), parse_url($falke_mdm_currDomain, PHP_URL_HOST), $tag);
	}else{
		return $tag;
	}
}
...
/*
add_filter( 'wp_get_attachment_image_src', 'falke_mdm_replace_url_in_imgsrc' );
function falke_mdm_replace_url_in_imgsrc($image){
	global $falke_mdm_currDomain;
	if(!is_admin()){
		$image[0] = str_replace(parse_url(get_site_url(), PHP_URL_HOST), parse_url($falke_mdm_currDomain, PHP_URL_HOST), $image[0]);
		return $image;
	}else{
		return $image;
	}
...
/*
add_filter( 'wp_calculate_image_srcset', 'falke_mdm_custom_image_srcset' );
function falke_mdm_custom_image_srcset( $sources ){
	global $falke_mdm_currDomain;
	if(!is_admin()){
		foreach($sources as $k => $v){
			$sources[$k]['url'] = str_replace(parse_url(get_site_url(), PHP_URL_HOST), parse_url($falke_mdm_currDomain, PHP_URL_HOST), $sources[$k]['url']);
		}
		return $sources;
	}else{
		return $sources;
